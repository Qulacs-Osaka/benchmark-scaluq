...
scaluq::Circuit<Prec> create_benchmark_circuit(std::uint64_t n_qubits,
  ...
  for (std::uint64_t layer = 0; layer < n_layers; ++layer) {
    for (std::uint64_t i = 0; i < n_qubits; ++i) {
      ...
      circuit.add_gate(scaluq::gate::RX<Prec>(i, dist(rng)));
      circuit.add_gate(scaluq::gate::RZ<Prec>(i, dist(rng)));
    }
  }
...
auto initialize_benchmark(const BenchmarkConfig &config) {
  scaluq::StateVectorBatched<Prec, Space> states(config.n_batches,
                                                 config.n_qubits);
  auto circuit = create_benchmark_circuit<Prec>(config.n_qubits,
                                                config.n_layers, config.seed);
