...
def transpile_on_gpu(qc):
    # Disable gate fusion and transpiler optimization to match the other
    # backends (plain qiskit-aer, Scaluq, Qulacs, cuStateVec), which all apply
    # the gates as-is without cross-gate fusion.
    backend = AerSimulator(method="statevector", device="GPU", precision="double", cuStateVec_enable=True, fusion_enable=False, max_job_size=1)
