...
def benchfunc(nqubits, eng, gate, reg):
    for __ in range(100):
        for t1 in range(nqubits-1):
            for t2 in range(nqubits):
                if t1 == t2: continue
                gate | (reg[t1], reg[t2])
    eng.flush()

def create_params(gates: list[tuple[str, Any]]):
...
@pytest.mark.parametrize(["name", "gate", "nqubits"], double_params)
def test_Double(benchmark, name, gate, nqubits):
    benchmark.group = name
    eng = MainEngine()
    reg = eng.allocate_qureg(nqubits)
    benchmark(benchfunc, nqubits, eng, gate, reg)
